...
/* 	.tabBox.layui-table th:nth-child(13){width:inherit!important;min-width:200px;}
	...
	.checkBoxTd th:first-child{width:90px!important;}
...
<div id="rrapp" v-cloak>
	...
	<div>	   	
		<!-- table -->
		 <table class='layui-table tabBox parentTab' style="margin:0 auto 20px auto;overflow-x: scroll;">
			<thead>
				<tr class="checkBoxTd">
					<th></th>
			...
			<tbody>
			   <tr class="s_addTr" v-for="(v,k) in cases">
			   		<td>
			   			<!-- <input class="clickChecked" type="checkbox"> -->
			   			<span class="check-one" data-num="0" onclick="selectData(this);"></span>
			   		</td>
   ...
   <script>
	...
	$(function(){
		/* 
		$('#fazhan-company').easyAutocomplete({
			minCharNumber : 1,// 至少需要1个字符
			url : function(phrase) {
				return base_url+'/transport/generalInfo/getCompanyListAndCurrCompanyInfo';
			},
   ...
   var vm = new Vue({
		...
		watch:{
			  ...
			  tabList:function(){
				  this.$nextTick(function(){
					  var _this = this;
					  var thLength = $(".parentTab.layui-table th").length;
					  //console.log(555);
					  //console.log(thLength); 
					  console.log('看看是否每次都执行');
		...
		methods: {
			...
			countTableWidth:function(length){
				...
				if(docWidth>=1300){
					/* if(length<=15){
						  $(".parentTab.layui-table").css("width","100%");
					  }else if(length>15 && length<=20){
						  $(".parentTab.layui-table").css("width","120%");
					  }else if(length>20 && length<=25){
						  $(".parentTab.layui-table").css("width","3800px");
					  }else if(length>25 && length<=30){
						  $(".parentTab.layui-table").css("width","180%");
					  }else if(length>30 && length<=36){
						  $(".parentTab.layui-table").css("width","210%");
					  }else if(length>36 && length<=42){
						  $(".parentTab.layui-table").css("width","240%");
					  }else if(length>42 && length<=46){
						  $(".parentTab.layui-table").css("width","280%");
					  }else if(length>46){
						  $(".parentTab.layui-table").css("width","335%");
					  } */
				}else{
					/* if(length<=10){
						  $(".parentTab.layui-table").css("width","80%");
					  }else if(length>10 && length<=15){
						  $(".parentTab.layui-table").css("width","110%");
					  }else if(length>15 && length<=18){
						  $(".parentTab.layui-table").css("width","130%");
					  }else if(length>18 && length<=23){
						  $(".parentTab.layui-table").css("width","160%");
					  }else if(length>23 && length<=26){
						  $(".parentTab.layui-table").css("width","180%");
					  }else if(length>26 && length<=30){
						  $(".parentTab.layui-table").css("width","215%");
						  ...
						  $(".parentTab.layui-table").css("width","350%");
					  }else if(length>46){
						  $(".parentTab.layui-table").css("width","400%");
					  } */
				}
				...
				$.ajax({
					   ...
					   success: function(data){
						 ...
						 if(data.success){
							 ...
							 vm.attrList=data.reportConfig.subList;
							 console.log(JSON.stringify(vm.attrList));
							 console.log(vm.attrList[0]);
							 console.log(vm.attrList.length);
							 if(vm.attrList.length<=0 || vm.attrList[0]=='undefined'){
								 $('.viewDetails').hide();
							 }else{
								 $('.viewDetails').show();
								 vm.lookName=data.reportConfig.subList[0].name;//渲染子菜单
								 sessionStorage.setItem("subListdata",JSON.stringify(data.reportConfig.subList));//存储进来时返回的url	 
							 }					 
						 }   
			...
			search:function(num){
				...
				ss.pageSize = this.pageSize;			
 				$.ajax({
					   url:base_url+'/report/query/data',
					   dataType:"json",
					   contentType:'application/json',
					   data:JSON.stringify(ss),
					   type:'post',
			...
			print:function(){
				...
				$.ajax({
					   url:base_url+'/report/export/data',
					   //dataType:"json",
					   contentType:'application/json',
					   data:JSON.stringify(ss),
					   type:'post',
   ...
   var timeout = setTimeout(function(){
	...
	function zibaobiao(){
		var indexNum=[];
		var getSubList=sessionStorage.getItem("subListdata");//获取储存的返回的数值
		console.log(getSubList);
		var paramsData=JSON.parse(getSubList)[0].params.split(',');
		console.log(paramsData);
		var parentKey=JSON.parse(getSubList)[0].reportConfigId;
		var getcaseList=sessionStorage.getItem("caseListData");//存储查询到的数据
		var getcaseLists=JSON.parse(getcaseList);
		console.log('lllllllllllllllllllllllllllllll');
		console.log(getcaseLists);
		$('.check-one').each(function(v,k){//循环所有单选框
			if($(this).attr('data-num')=='1'){//单选选中状态
				console.log("=======");
				console.log(v);//2
				$.each(getcaseLists,function(s,m){//循环所有存储的数据
					console.log(s);
					if(s==v){//找到单选框对应的那一条
						console.log("===");	
						console.log(m);
						indexNum.push(m);//找到单选框对应的那一条放到一个新数组里
					}
				})
			}
		})
		...
		if(indexNum.length>0){
			$.each(paramsData,function(k,w){//循环刚开始进来缓存返回的url
				$.each(indexNum[0],function(key,value){
					if(key == w){
						str +="&" + w + "=" + value;//需拼接到url后面的
					}
				})
			})
		}else{
	...
	if(indexNum.length=="1"){//判断如果选中的是一条
		var hwy_ydbhid=indexNum[0].hwy_ydbhid;//线路
		var xianlu=indexNum[0].xianlu;//财务凭证
		var linkUrl=JSON.parse(getSubList)[0].linkUrl;//获取url
		console.log(linkUrl);
			...
			layer.open({
				 ...
				 // time: 2000, //2秒后自动关闭
				  anim: 2,
				  content: [base_url+linkUrl + "&parentKey=" + parentKey + str, 'yes'], //iframe的url，no代表不显示滚动条
				  end: function () {
            	  ...
            	  success: function(layero, index){
	                ...
	                var parmsUrl =body.find('.parmsUrl').text();//获取url
        		    var params = body.find('.parmsUrl').text().split("&");//进行分割成数组
        		    var obj = {};//声明对象
        		    ...
        		    console.log(obj);
        		    console.log(obj['hwyd.ydbhid']);
        		    body.find('input[data="hwyd.ydbhid"]').val(obj['hwyd.ydbhid']);//运单号
        		    body.find('input[data="的fhshj"]').val("");//日期清空
        		    /* if(obj['hwyd.ydbhid']!=""||obj['hwyd.ydbhid']!="undefined"){
        		    	body.find('#searchBtn').click();
        		    } */   //模拟点击它会调父页面同一个查询接口，导致第二次缓存的选中对应数据是子页面的，父页面选中对应的数据被替换  		     */	
	               }
				});
