...
public class TransportDubboServer {

	/** slf4j logger api */
	protected static final Logger logger = LoggerFactory.getLogger(TransportDubboServer.class);
    ...
    public static void main(String[] args) {
        try {
            ...
            Runtime.getRuntime().addShutdownHook(new Thread() {
                public void run() {
                    ...
                    synchronized (TransportDubboServer.class) {
                        running = false;
                        TransportDubboServer.class.notify();
                    }
                }
            });
        ...
        synchronized (TransportDubboServer.class) {
            while (running) {
                try {
                	TransportDubboServer.class.wait();
                } catch (Throwable e) { }
            }
        }
    }
