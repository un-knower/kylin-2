...
import com.ycgwl.kylin.transport.entity.FkfshResult;
import com.ycgwl.kylin.transport.entity.HwydRoute;
import com.ycgwl.kylin.transport.entity.OnTheWay;
import com.ycgwl.kylin.transport.entity.ReorderConfigure;
import com.ycgwl.kylin.transport.entity.ShqsdResult;
import com.ycgwl.kylin.transport.entity.TransportOrder;
import com.ycgwl.kylin.transport.entity.TransportOrderTrace;
import com.ycgwl.kylin.transport.entity.TransportProcessStock;
import com.ycgwl.kylin.transport.entity.TransportQsztBase;
import com.ycgwl.kylin.transport.entity.TransportSignRecord;
import com.ycgwl.kylin.transport.entity.TransportSignRecordEntity;
import com.ycgwl.kylin.transport.entity.TransportSignRecordLog;
import com.ycgwl.kylin.transport.entity.TransportSignRecordResult;
...
import com.ycgwl.kylin.transport.persistent.BundleReceiptMapper;
...
import com.ycgwl.kylin.transport.persistent.TransportSignRecordMapper;
...
public class TransportSignRecordServiceImpl implements ITransportSignRecordService,
    AuthorticationConstant {
  ...
  public String fullTicketReceipt(TransportSignRecordEntity transportSignRecordEntity)
...
//		}
    ...
    if (signRecord.getQszt() > 0) {
      ...
      try {
        ...
        if (transportSignRecordOld != null && "1"
            .equals(transportSignRecordOld.getCxFlag())) {//撤销过签收，先删除撤销信息，再修改签收时间
...
//					this.modifyTransportSignRecordByYdbhid(transportSignRecord);
      ...
      List<BundleReceipt> bundleReceiptList = bundleReceiptMapper
          .queryBundleReceiptByYdbhidDesc(ydbhid);
      ...
      for (BundleReceipt bundleReceipt : bundleReceiptList) {
        if (bundleReceipt.getiType() == 0 && bundleReceipt.getYdzh() == 0 && "中转"
            .equals(bundleReceipt.getTransferFlag())) {
      ...
      BundleReceipt bundle = bundleReceiptService
          .getLastBundleReceiptByYdbhid(signRecord.getYdbhid()).get(0);
...
//			}
      if (signRecord.getQsTime().getTime() <= fchrqTime.getTime()) {
        throw new ParameterException(
            "签收时间不能小于等于装载发车时间：" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(fchrqTime));
      }
      if (order.getFhshj().getTime() >= signRecord.getQsTime().getTime()) {
        throw new ParameterException(
            "签收时间不能小于等于运单托运时间：" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                .format(order.getFhshj()));
      }
      if (isArrive && dhsj != null && signRecord.getQsTime().getTime() <= dhsj.getTime()) {
        throw new ParameterException(
            "签收时间要大于到站时间:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(dhsj));
      }
  ...
  @Override
  @Transactional
  public TransportSignRecordResult canTransportOrderSign(
      TransportSignRecordSearch transportSignRecordSearch)
      throws ParameterException, BusinessException {
    ...
    Assert.trueIsWrong("运单号不是10位或12位是不正确的", !(ydbhid.length() == 10 || ydbhid.length() == 12),
        "请输入正确的运单编号！");
...
//		}
    ...
    List<BundleReceipt> bundleReceiptList = bundleReceiptService
        .getLastBundleReceiptByYdbhid(ydbhid);//根据运单号查询装载清单（装车时间倒序排）
    Assert.trueIsWrong("没有做过装载清单", bundleReceiptList == null || bundleReceiptList.size() == 0,
        "该运单没有做装载操作！");
    ...
    Assert.trueIsWrong("fiwtResult == null ",
        fiwtResult == null || StringUtils.isEmpty(fiwtResult.getXianlu()), "当前运单没有做财凭，不能签收！");
    ...
    Assert.trueIsWrong("isTransferAndNotArrive is true", isTransfer && arriveStatus,
        "干线中转的装载请先做到货，才能签收！");
...
//				throw new ParameterException("此运单号要求到站付款或代收款,签收前请生成分理收据");
    ...
    if (!transportSignRecordSearch.getCompany().equals(transportOrder.getFazhan()) &&
      ...
      if (transportOrder.getDzfk() != null && transportOrder.getDzfk() == 1) {  //判断是否是到站付款
        //查询是否生成提货单和派车单
        if (carOutResult != null || thqshdid != null || "1"
            .equals(transportSignRecord.getDaoZhanOk())) { //已生成派车单或者提货单，装载清单是否到站
          if (payInfo.getDsk() + payInfo.getHdfk() > 0) {//到付，判断是否打印收据
            ...
            if (sjidList.isEmpty() || sjidList.size() == 0) {//未打印收据，判断是否生成收据
              if (StringUtils.isEmpty(this.getCarOutShjhmByYdbhid(ydbhid)) && StringUtils
                  .isEmpty(this.getThqsdShjhmByYdbhid(ydbhid))) {
...
//				}	
    ...
    //返回页面数据
    if (transportSignRecord != null && transportSignRecord.getQszt() != null
        && transportSignRecord.getQszt() != 0) {//已签收
      ...
      transportSignRecordResult
          .setQsTime(DateUtils.getCurrentHourAndMinutesTime(transportSignRecord.getQsTime()));
      ...
      transportSignRecordResult.setLrTime(transportSignRecord.getLrTime() == null ? ""
          : DateUtils.getCurrentHourAndMinutesTime(transportSignRecord.getLrTime()));
    } else if (transportSignRecord != null
        && transportSignRecord.getOldQsTime() != null) {//未签收，但做过撤销签收
      /**
       * 新增需求：第一次签收默认系统时间，当月内撤销后可以更改签收时间和签收状态，跨月后显示当月内最后一次签收的时间和状态
       * 2018-02-07 11:36
       * yanxif
      ...
      if (transportSignRecord.getOldQsTime() == null || DateUtils
          .isTimeInterval(transportSignRecord.getOldQsTime(), new Date())) {//可以更改签收时间
        ...
        transportSignRecordResult
            .setQsTime(DateUtils.getCurrentTimeStr(transportSignRecord.getOldQsTime()));
        if (transportSignRecord.getRecordQsTime() != null) {
          transportSignRecordResult.setQsTime(
              DateUtils.getCurrentHourAndMinutesTime(transportSignRecord.getRecordQsTime()));
        }
      }
    } else {//未签收，且未做过撤销签收
      transportSignRecordResult.setIsSign(false);
    ...
    if (carOutResult != null && carOutResult.getPcshlc() > 0) {
      ...
      transportSignRecordResult
          .setLc(carOutResult.getMin().toString() + '-' + carOutResult.getMax().toString());
    } else {
   ...
   */
  public void transportSignRecordLogConversions(TransportSignRecord transportSignRecord,
      TransportSignRecordLog transportSignRecordLog) {
  ...
  public void removeHwydRouteByYdbhid(String ydbhid) throws ParameterException, BusinessException {
    ...
    try {
      transportSignRecordMapper
          .deleteHwydRouteByYdbhid(ydbhid, TransportSignRecordServiceImpl.REVOCATION_SIGN_IN);
    } catch (Exception e) {
  ...
  @Override
  public ShqsdResult getReachAfterPayInfo(String ydbhid)
      throws ParameterException, BusinessException {
    if (StringUtils.isEmpty(ydbhid)) {
      throw new ParameterException("ydbhid", ydbhid, "运单编号不能为空");
    }
  ...
  @Override
  public CarOutResult getCarOutByYdbhid(String ydbhid)
      throws ParameterException, BusinessException {
    if (StringUtils.isEmpty(ydbhid)) {
      throw new ParameterException("ydbhid", ydbhid, "运单编号不能为空");
    }
  ...
  @Override
  public List<String> getCwsjDetailByYshm(String yshm)
      throws ParameterException, BusinessException {
  ...
  @Override
  public TransportSignRecordResult searchUndoSign(
      TransportSignRecordSearch transportSignRecordSearch)
      throws ParameterException, BusinessException {
    ...
    //判断用户单据撤销超级权限
    Integer permission = transportRightMapper.getRightNum(AuthorticationConstant.SUP_CANCEL_RIGHT,
        transportSignRecordSearch.getAccount());

    if ((null == permission) || (0 == permission)) {
      //判断是否是本公司用户撤销签收
      if (!(transportSignRecordSearch.getCompany().equals(transportOrder.getFazhan())
          || transportSignRecordSearch.getCompany().equals(transportOrder.getDaozhan())
          || transportSignRecordSearch.getCompany()
          .equals(TransportSignRecordServiceImpl.COMPANY_HEADQUARTER))) {
  ...
  @Override
  public void removeTransportOrderTraceByYdbhid(String ydbhid)
      throws ParameterException, BusinessException {
    if (StringUtils.isEmpty(ydbhid)) {
      throw new ParameterException("ydbhid", ydbhid, "运单编号不能为空");
    }
  ...
  @Override
  @Transactional(rollbackFor = {ParameterException.class, BusinessException.class})
  public String undoSign(TransportSignRecordSearch transportSignRecordSearch)
      throws ParameterException, BusinessException {
    // 查询用户权限
    Integer permission = transportRightMapper
        .getRightNum(AuthorticationConstant.HCZZQD_CANCEL, transportSignRecordSearch.getAccount());
    if ((null == permission) || (0 == permission)) {
      return null;
    }
    ...
    if (transportSignRecord != null && (qszt == 1 || qszt == 2 || qszt == 3 || qszt == 20
        || qszt == 21)) {
      String flag = "";
      try {
    ...
    undoTransportSignRecord.setCxNumber(
        transportSignRecord.getCxNumber() == null ? 1 : transportSignRecord.getCxNumber() + 1);
    try {
    ...
    if (transportSignRecord.getOldQsTime() != null) {
      operatingContent = "运单编号为：" + ydbhid + "的签收记录撤销，第一次签收时间为：" + DateUtils
          .getCurrentTimeStr(transportSignRecord.getOldQsTime()) + ",再次签收时将不可更改！";
    } else {
  ...
  public boolean batchTransportSign(TransportSignRecordEntity transportSignRecordEntity)
    ...
    //5）判断是否是本公司用户签收
    if (bundleReceipt.getFazhan().equals(transportOrder.getFazhan()) || bundleReceipt.getDaozhan()
        .equals(transportOrder.getDaozhan())) {
      if (!(transportSignRecordEntity.getCompany().equals(transportOrder.getFazhan())
          || transportSignRecordEntity.getCompany().equals(transportOrder.getDaozhan()))) {
        throw new ParameterException(
            "本票货当前到站是“" + transportOrder.getDaozhan() + "”不是本公司,禁止录入客户签收！");
      }
    } else {
    ...
    if (bundleReceipt.getiType() == 0 && TransportSignRecordServiceImpl.TRANSFER_YES
        .equals(bundleReceipt.getTransferFlag()) && bundleReceipt.getYdzh() == 0) {
      //干线中转且未到货
      throw new ParameterException("该运单需要先做到站处理才能签收！");
    }

    if ((bundleReceipt.getiType() == 0 && TransportSignRecordServiceImpl.TRANSFER_NO
        .equals(bundleReceipt.getTransferFlag()) && bundleReceipt.getYdzh() == 0) ||
        (bundleReceipt.getiType() == 1 && bundleReceipt.getYdzh() == 0) ||
        (bundleReceipt.getiType() == 0 && (bundleReceipt.getTransferFlag() == null
            || bundleReceipt.getTransferFlag() == "") && bundleReceipt.getYdzh() == 0)) {
      ...
      try {
        requestJsonEntity.put("storeHouse",
            adjunctSomethingMapper.getStoreHouseBycompany(transportSignRecordEntity.getCompany())
                .get(0));
        /**
    ...
    //先判断是否撤销过签收
    TransportSignRecord transportSignRecordOld = this
        .getTransportSignRecordByYdbhid(signRecord.getYdbhid());
    ...
    Date fchrqTime;
    try {
      fchrqTime = bundleReceiptService.getLastBundleReceiptByYdbhid(signRecord.getYdbhid()).get(0)
          .getFchrq();
    } catch (Exception e) {
    ...
    Date firstDayOfMonth = DateUtils.getFirstDayOfMonth();//当月1号
    if (signRecord.getQsTime().getTime() <= fchrqTime.getTime()
        || signRecord.getQsTime().getTime() <= firstDayOfMonth.getTime()) {
      throw new ParameterException("签收时间不能小于等于发车时间,也不能小于当月1号!");
    }
...
//		}
    ...
    BundleReceipt bundle = bundleReceiptService.getLastBundleReceiptByYdbhid(signRecord.getYdbhid())
        .get(0);
...
//		}
    if (isArrive && dhsj != null && signRecord.getQsTime().getTime() < dhsj.getTime()) {
      throw new ParameterException(
          "签收时间要不能小于到站时间:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(dhsj));
    }
    if (signRecord.getQsTime().getTime() > new Date().getTime()) {
      throw new ParameterException("签收时间不能大于当前系统时间");
    }
    ...
    if (signRecord.getQsTime().getTime() <= order.getFhshj().getTime()) {
      throw new ParameterException(
          "签收时间要大于运单托运时间:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(order.getFhshj()));
    }
...
//		}
  ...
  public JsonResult saveReorder(String account, ReorderDto reorderDto) {
    TransportSignRecord transportSignRecord = transportSignRecordMapper
        .selectTransportSignRecordByYdbhid(reorderDto.getWaybillNum());
...
//		}
  ...
  public JsonResult selectReorderConfigure(String account) {
    // 0. 是否有权限
    // 1.判断是否返单参数管理权限
    Integer rightNum = transportRightMapper
        .getRightNum(AuthorticationConstant.FD_YJ_CANSHU, account);
    if (rightNum == null || rightNum < 1) {
      return JsonResult.getConveyResult("400", "你没有权限使用这个功能，请与系统管理员联系！");
    }
  ...
  public JsonResult selectReceiveReorder(String account, String company, ReorderDto reorderDto) {
    ...
    // 4. 查询
    ReorderVo reorderVo = transportSignRecordMapper
        .selectReceiveReorder(reorderDto.getWaybillNum(), company);
    if (null == reorderVo) {
